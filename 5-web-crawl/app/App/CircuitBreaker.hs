...
-- | Module containing circuit breaker functionality, which is the ability to
-- open a circuit once a number of failures have occurred, thereby preventing
-- later calls from attempting to make unsuccessful calls.
-- | Often this is useful if the underlying service were to repeatedly time out,
-- so as to reduce the number of calls inflight holding up upstream callers.
module App.CircuitBreaker(
...
isCircuitBreakerOpen :: (MonadUnliftIO m) => CircuitBreakerState -> m Bool
...
isCircuitBreakerClosed :: (MonadUnliftIO m) => CircuitBreakerState -> m Bool
...
circuitBreaker :: (MonadThrow m, MonadUnliftIO m, MonadUnliftIO n)
...
circuitBreaker options service =
  let getCurrentTime              = liftIO (round `fmap` getPOSIXTime)
      ...
      failingCall                 = throwM $ CircuitBreakerException $ breakerDescription options
